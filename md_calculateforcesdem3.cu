...
__device__ void dd_CheckBreakConditionDEM_2(float& Sc, float& Mc, float3* __restrict__ Oijt,	float3* __restrict__ Mijn, float3* __restrict__ Mijt,
	uint* __restrict__ ILtype, const uint& kdx, const float m_Gcritn, const float m_Gcritt, const float b_r)
{
	Mc *= norm3df(Oijt[kdx].x, Oijt[kdx].y, Oijt[kdx].z);
	Sc += norm3df(Mijt[kdx].x, Mijt[kdx].y, Mijt[kdx].z) * 4.0f * b_r;
	//float Sc2 = norm3df(Mijt[kdx].x, Mijt[kdx].y, Mijt[kdx].z) * 4.0f * b_r;
	Mc += norm3df(Mijn[kdx].x, Mijn[kdx].y, Mijn[kdx].z) * 2.0f * b_r;
	//float Mc2 = norm3df(Mijn[kdx].x, Mijn[kdx].y, Mijn[kdx].z) * 2.0f * b_r;
	//if ((Sc > m_Gcritn) || (Sc2 > m_Gcritn) || (Mc > m_Gcritt) || (Mc2 > m_Gcritt))
...
__device__ void dd_Calculate_rijm_nij_2(const float* __restrict__ R, const uint& idx, const uint& jdx, const uint& N, float3& nij, float& rijm)
{
	float3 rij;
	float _1d_rm;
	rij.x = R[jdx] - R[idx];
...
__device__ void dd_Calculate_M123_bpm_2(const float* __restrict__ R, const float* __restrict__ W, float3* __restrict__ Rij, const uint& kdx, const uint& idx, const uint& jdx, const uint& N, const float dt, const float3& nij, float3& M1, float3& M2, float3& M3)
{
...
__device__ void dd_Calculate_fijn_bpm_2(const float* __restrict__ _1d_iL, float& Sc, float3& Fijn, const uint& kdx, const float3& nij, const float& rijm, const float& m_E, const float& b_r)
{
	// calculate the force
...
__device__ void dd_Calculate_mijn_bpm_2(const float* __restrict__ W, const float* __restrict__ _1d_iL, float3* __restrict__ Mijn, float3& Mijnn, const uint& kdx, const uint& idx, const uint& jdx, const uint& N,
...
__device__ void dd_Calculate_mijt_bpm_2(const float* __restrict__ W, const float* __restrict__ _1d_iL, float3* __restrict__ Mijt, float3& Mijtt, const uint& kdx, const uint& idx, const uint& jdx, const uint& N,
...
__device__ void dd_Calculate_fijt_mijunsym_bpm_2(const float* __restrict__ R, const float* __restrict__ V, const float* __restrict__ W, 
	const float* __restrict__ _1d_iL, float3* __restrict__ Oijt, float& Mc, float3& Fijt, float3& Mijadd, const uint& kdx, const uint& idx, const uint& jdx, const uint& N,
...
__device__ void dd_Calculate_fijn_ppHM_2(const float* __restrict__ V, const float* __restrict__ W, float3& Fijn,
	const uint& kdx, const uint& idx, const uint& jdx, const uint& N,
...
__device__ void dd_Calculate_fijtmijt_ppHM_2(float3* __restrict__ Oijt, float3& Fijt, float3& Mijt,
	const uint& kdx, const float3& nij, const float& m_mu, const float& p_A, const float& mP, const float& rP, const float& dt,
...
__device__ void dd_Calculate_mijroll_ppHM_2(const float* __restrict__ W, float3& Mijadd, const uint& kdx, const uint& idx, const uint& N, const float& m_muroll, const float& fijnm)
{
	float3 wi;
	wi.x = W[idx];
	wi.y = W[idx + N];
	...
	const float m_mu, const float m_muroll, const float mP, const float rP)
{
	//float rr, fm, _1d_r;
	//float3 dr, fs;
	uint idx = blockIdx.x * blockDim.x + threadIdx.x, jdx, kdx;
	...
	{		
		...
		for (kdx = idx * IonP; kdx < (idx + 1) * IonP; ++kdx)
		{
			...
			if (jdx != UINT_MAX && ILtype[kdx] == 1)
			{
				...
				//printf("In %u %u %u | %e %e %e %e %e | %e\n", kdx, idx, jdx, Sc, Mc, m_Gcritn, m_Gcritt, b_r, norm3df(Mijt[kdx].x, Mijt[kdx].y, Mijt[kdx].z));
