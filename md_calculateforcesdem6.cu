...
__device__ void dd_Calculate_rijm_nij_5(const float* __restrict__ R, const uint& idx, const uint& jdx, const uint& N, float3& nij, float& rijm)
{
	float3 rij;
	float _1d_rm;
	rij.x = R[jdx] - R[idx];
...
__device__ void dd_Calculate_M123_bpm_5(const float* __restrict__ R, const float* __restrict__ W, float3* __restrict__ Rij, const uint& kdx, const uint& idx, const uint& jdx, const uint& N, const float dt, const float3& nij, float3& M1, float3& M2, float3& M3)
{
...
__device__ void dd_Calculate_fijn_bpm_5(const float* __restrict__ _1d_iL, float& Sc, float3& Fijn, const uint& kdx, const float3& nij, const float& rijm, const float& m_E, const float& b_r)
{
	// calculate the force
...
__device__ void dd_Calculate_mijn_bpm_5(const float* __restrict__ W, const float* __restrict__ _1d_iL, float3* __restrict__ Mijn, float3& Mijnn, const uint& kdx, const uint& idx, const uint& jdx, const uint& N,
...
__device__ void dd_Calculate_mijt_bpm_5(const float* __restrict__ W, const float* __restrict__ _1d_iL, float3* __restrict__ Mijt, float3& Mijtt, const uint& kdx, const uint& idx, const uint& jdx, const uint& N,
...
__device__ void dd_Calculate_mijroll_ppHM_5(const float* __restrict__ W, float3& Mijadd, const uint& kdx, const uint& idx, const uint& N, const float& m_muroll, const float& fijnm)
{
	float3 wi;
	wi.x = W[idx];
	wi.y = W[idx + N];
