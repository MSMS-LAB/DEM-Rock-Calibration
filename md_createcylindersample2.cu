...
{   
	...
	l_StepsIncreaseSample= 3000*int(Fire.Tsim/Fire.dt0), l_StepsPushSample[3]= {2000*int(Fire.Tsim/Fire.dt0),4000*int(Fire.Tsim/Fire.dt0),100*int(Fire.Tsim/Fire.dt0)},
	l_StepsReduceParticlesNumber= 500*int(Fire.Tsim/Fire.dt0), l_StepsIntervalReduceParticlesNumber= 500*int(Fire.Tsim/Fire.dt0),
	l_StepsAddParticle = 3000*int(Fire.Tsim/Fire.dt0);
	double l_StepIntervalAddParticleD = l_StepsAddParticle/double(mdTD.P.N);
	if(l_StepIntervalAddParticleD<1) {l_StepsAddParticle = mdTD.P.N; l_StepIntervalAddParticleD = 1.0;}
	
	
	uint l_StepIntervalAddParticle = uint(l_StepIntervalAddParticleD);
	
	
	std::cerr<<"T "<<int(Fire.Tsim/Fire.dt0)<<" "<<Fire.StepsPreRelaxation<<" "<<l_StepsIncreaseSample<<" "<<l_StepsReduceParticlesNumber<<"\n";
	
	double l_maxintersection = 0, l_maxpredeformation=-0.1, l_maxdeformation=-0.01;
	uint l_deletedparticles=0;
	for (steps = 0; steps < Fire.StepsPreRelaxation || l_maxintersection<l_maxpredeformation; ++steps)
	{		
		if (steps % l_StepsSaveLammps  == 0)
		{
			HANDLE_ERROR(cudaMemcpy(mdTD.P.h_R, mdTD.P.d_R, 3 * mdTD.P.N * sizeof(float), cudaMemcpyDeviceToHost));
			HANDLE_ERROR(cudaMemcpy(mdTD.IL.h_IL, mdTD.IL.d_IL, mdTD.IL.N * sizeof(uint), cudaMemcpyDeviceToHost));
			HANDLE_ERROR(cudaMemcpy(mdTD.IL.h_ILtype, mdTD.IL.d_ILtype, mdTD.IL.N * sizeof(uint), cudaMemcpyDeviceToHost));
		...
		//if (steps % l_StepsIncreaseD == 0 && mdTD.Po.D < 0.3e-4 * dmax)	mdTD.Po.D *= 1.30;
		...
		if (steps % l_StepIntervalAddParticle== 0)
		{	
		...
		d_CylinderRestrictionZ << <mdTD.A.bloks, SMEMDIM >> > (mdTD.P.d_V, mdTD.P.d_R, mdTD.P.N, mdTD.S.center, spacesized2.x, spacesized2.z, mdTD.S.hidenpoint.z);// std::cerr << "e4 " << steps << "\n";
	...
	{	
		...
		d_CylinderRestrictionZ << <mdTD.A.bloks, SMEMDIM >> > (mdTD.P.d_V, mdTD.P.d_R, mdTD.P.N, mdTD.S.center, mdTD.S.sized2.x, mdTD.S.sized2.z, mdTD.S.hidenpoint.z);
	...
	std::cerr<<"Result porosity "<<(mdTD.P.N-l_deletedparticles)*mdTD.Po.p_V/mpMDP.S[isample].Vext<<"\n";//std::cin.get();
...
void h_LoadCylinderSample(mp_mdparameters_data& mpMDP, uint isample)
{
	char filename[256] = "";
...
void h_LoadCylinderSample(mp_mdparameters_data& mpMDP, uint isample, char* filename)
{	
	LoadParticleDATAtxt(mpMDP.h_R[isample], mpMDP.PN[isample], filename);
	mpMDP.S[isample].PN = mpMDP.PN[isample];
}
